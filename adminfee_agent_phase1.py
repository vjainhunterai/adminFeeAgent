...
from langgraph_test import result
...
#llm = ChatOpenAI(model="gpt-4.1-mini") // disabled for replacing with llama model
...
def contract_input_node(state:AgentState) -> AgentState:
    ...
    if input_type == "file":
        print("\nPlease place excel file in below location:\n")
        print(FILE_PATH + "\n")
...
def completion_node(state: AgentState):
    ...
    output_path = f"/home/ubuntu/adminfee_pieline/Data/excel_output/{delivery}"

    print("\n Admin Fee Processing Completed successfully!")
...
def confirmation_node(state: AgentState) -> AgentState:
    ...
    print("\nNext Phase: Monitoring job status (phase-3).")
...
def contract_summary_node(state: AgentState):
    ...
    for contract in contracts:
        ...
        with engine.begin() as conn:
            po_result = conn.execute(text(po_sql)).fetchall()

            report_result = conn.execute(text(report_sql), {"contract_id":contract}).fetchall()
